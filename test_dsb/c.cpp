...
case RequestType::COMPOSE:
    if(td->nbHops == 1){
      XBT_DEBUG("Output Function for COMPOSE, put to home_timeline_service");
...
/* PR FUNCTIONS, AUTO GENERATED CODE, MODIFY IF YOU KNOW WHAT YOU WANT */
    double pr_nginx_web_server(TaskDescription* td) {
      ...
      {
case RequestType::COMPOSE:
    if(td->nbHops == 1){XBT_DEBUG("Entered cost Function for COMPOSE"); return 1564000;}
...
return -1;
    ...
    double pr_home_timeline_service(TaskDescription* td) {
      ...
      {
case RequestType::COMPOSE:
    if(td->nbHops == 2){XBT_DEBUG("Entered cost Function for COMPOSE"); return 1142000;}
